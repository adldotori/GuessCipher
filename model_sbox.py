...
if __name__ == '__main__':
    ...
    model_A = Model(48, 512, known_key_bits, 1)  # **********************
    model_B = Model(512, 32, known_key_bits, 0)
    
    model_A.to(device)
    model_B.to(device)

    optimizer_A = torch.optim.Adam(model_A.parameters(), lr=0.00005) 
    optimizer_B = torch.optim.Adam(model_B.parameters(), lr=0.00005) 
    ...
    while(True):
        ...
        model_A.eval()
        for count_B in range(10):

            for _ in range(1000):
                tmp_plain, tmp_cipher = make_train_data_sbox() # **********************
