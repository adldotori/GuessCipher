...
def make_tarin_data(rounds):
    cipher = des(rounds)
    key = os.urandom(8)
    active_key = cipher.permut(string_to_bit_array(key), CP_1)
    plaintext = os.urandom(8)
    ciphertext = cipher.encrypt(key, plaintext)
    train_plain = make_data(string_to_bit_array(plaintext))
    train_cipher = make_data(string_to_bit_array(ciphertext))
    train_key = make_data(active_key)
    return train_plain, train_cipher, train_key

def make_train_data_IP():
    cipher = des(1)
    plaintext = string_to_bit_array(os.urandom(8))
    ciphertext = cipher.permut(plaintext,PI)
    train_plain = (plaintext)
...
def make_train_data_simple2():
    cipher = des(1)
    plaintext = os.urandom(6)
    key = os.urandom(6)
    train_key = string_to_bit_array(key)
    train_plain = string_to_bit_array(plaintext)
    train_cipher = cipher.permut(cipher.substitute(cipher.xor(train_plain, train_key)), P)
